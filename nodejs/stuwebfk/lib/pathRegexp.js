// 路由规则转换成正则表达式，然后去匹配url是否符合该路由规则

var routeRule = '/contact/*/:id/ok';

module.exports = pathRegexp;

function pathRegexp(path) {
    var paramNames = [];     // 正则捕获组  /about/:name/:age 要得到一个数组 ["name","age"];

    path = path

        // 把 * 替换成正则表达式的 [0-9a-zA-Z\-_]* 形式
        .replace(/((\*{1}(?=\/))|(\*{1}(?=$)))/g, "[0-9a-zA-Z\-_]*")

        // 把 :xxx 的形式替换成  [0-9a-zA-Z\-_]*  正则表达式形式
        ...
        // 把 / 转换为 \/ ,因为这是字符串形式，最后通过 new RegExp(path)生成时，必须要经过这个转换。
        .replace(/\//g, "\\\/")
