...
// 字段信息保存到req.body,二进制信息即上传文件信息保存到req.files[fileName]中
...
module.exports = function post(req,res,next){
    ...
    req.on("data", chunk => chunk_list.push(chunk))
    req.on("end", () => {
        ...
        if(isMulti){
            ...
            var readState = 0;
            var body = [];     // body体数据
            var position = 0;  // 正在读取的body_data的位置
            ...
            try{
                req.body = require("querystring").parse(body_data);
            }catch(e){}
        }
